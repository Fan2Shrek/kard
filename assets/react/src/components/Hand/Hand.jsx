...
import s from 'react-sortablejs';
...
export default forwardRef(({ hand, canPlay, order = null, gameActions = null }, ref) => {
	...
	return <div ref={ref} className='hand__container'>
		...
		{error && <div className='error'>{error}</div>}
			<s.ReactSortable list={cards} setList={setCards} className='hand'>
				{cards.map((card, index) => {
					return <div style={{ zIndex: index }}>
					<Card  onClick={handleCard} selected={selectedCards.includes(card)} key={`${card.rank}-${card.suit}`} card={card} img={getCardAsset(card)} angle={0} />
					</div>
				})}
			</s.ReactSortable>
